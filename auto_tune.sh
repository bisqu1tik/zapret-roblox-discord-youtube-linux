...
check_roblox_main() {
    local tmpfile=$(mktemp)
    local code=$(curl -s --tlsv1.3 --connect-timeout "$CURL_TIMEOUT" --max-time "$CURL_TIMEOUT" \
        -o "$tmpfile" -w "%{http_code}" "https://www.roblox.com" 2>/dev/null)
    local size=$(wc -c < "$tmpfile" 2>/dev/null || echo 0)
    local has_keyword=0
    grep -qi "roblox" "$tmpfile" 2>/dev/null && has_keyword=1
    rm -f "$tmpfile"
    [[ "$code" =~ ^[23] ]] && [[ $has_keyword -eq 1 ]]
}
...
test_strategy() {
    local yt_ok=0 cdn_ok=0 rb_ok=0
    if check_youtube_main; then
        yt_ok=1
...
if check_youtube_full; then
	if check_roblox_main; then
		echo -e "${GREEN}✓${NC} Roblox и YouTube доступны.. Ничего делать не нужно."
	else
		echo -e "${CYAN}~${NC} YouTube уже доступен но Roblox нет."
	fi
	echo "В любом случае запустить приложение? [y,N]:"
	read _startAn
	if [[ -z "$_startAn" ]]; then
		exit 1
	fi
...
for ((i=1; i<=MAX_STRATEGY; i++)); do
	...
	IFS=':' read -r yt_ok cdn_ok rb_ok <<< "$result"

	if [[ $rb_ok -gt 0 ]]; then
		echo -e "${GREEN}Roblox:✓${NC}"
	else
		echo -e "${RED}Roblox:✗${NC}"
	fi
